...
const serviceAccount = require('./serviceAccountKey.json'); // Asegúrate de tener tu llave de Firebase
...
// IMPORTAMOS LOS DATOS QUE YA TENEMOS
const catalog = {
...
async function uploadCatalog() {
    console.log("Iniciando carga de 82 productos a Firestore...");
    
    const batch = db.batch();

    // Procesar Webs
    catalog.web.forEach(p => {
        const ref = db.collection('products').doc(p.id);
        batch.set(ref, p);
    });

    // Procesar IA
    catalog.ia.forEach(p => {
        const ref = db.collection('products').doc(p.id);
        batch.set(ref, p);
    });

    // Procesar Seguridad
    catalog.security.forEach(p => {
        const ref = db.collection('products').doc(p.id);
        batch.set(ref, p);
    });

    await batch.commit();
    console.log("¡ÉXITO! 82 productos sincronizados en Firestore.");
}
